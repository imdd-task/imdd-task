...
def get_graylabel(M_bits: int) -> np.ndarray:
    """ """
    if M_bits == 1:
        return np.array([[0], [1]], dtype='uint8')
    graybits = get_graylabel(M_bits - 1)
    lower = np.hstack(
        (np.zeros((2**(M_bits - 1), 1), dtype='uint8'), graybits))
    upper = np.hstack(
        (np.ones((2**(M_bits - 1), 1), dtype='uint8'), np.flipud(graybits)))
...
def root_raised_cosine(
    ...
    return torch.sqrt(torch.tensor(
        raised_cosine(rolloff, samples_per_symbol, freq)).float())
